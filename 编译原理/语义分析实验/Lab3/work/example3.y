...
ConstArrIdex : /*empty*/ {$$ = newNode("ConstArrIdex");}
    |'[' ConstExp ']' ConstArrIdex{
    fprintf(fDetail, "ConstArrIdex -> [ ConstExp ] ConstArrIdex\n");
...
Entry_Void_Para: /*empty*/ {
    $$ = newNode("Entry_Void_Para");
	}
	;

Entry_Int_Para: /*empty*/ {
    $$ = newNode("Entry_Int_Para");
	}
	;

FuncFParam : INT Ident ParaArr{
    fprintf(fDetail, "FuncFParam -> int Ident ParaArr\n");
    $$ = newNode("FuncFParam");
    addChild($$, newNode("int"));
    addChild($$, newNode($2));
    addChild($$, $3);
