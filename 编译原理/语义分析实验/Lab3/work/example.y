        ...
        if(isVarInTable($1)){
            ...
            if(level == 0){ // 全局变量
                ...
                for(auto v: nodes[$4].values){
                    var.values2.push_back(nodes[v].value);
                }
                insertVar($1, var);
            }else{
...
ConstArrIdex : '[' ConstExp ']'{
    ...
    addChild($$, newNode("]"));
        ...
        nodes[$$].values.insert(nodes[$$].values.end(), nodes[$1].values.begin(), $1->values.end());
    ...
    /* empty */ {
        ...
        for(int i=0;i<paramList.size();i++){
            if(paramList[i]->isArr){
                insertVar(paramList[i]->label, Var(Addr, 0, 32+curSize, false, paramList[i]->values));
                curSize += 8;
            }else{
                insertVar(paramList[i]->label, Var(Int, 0, 32+curSize, false));
                curSize += 4;
            }
        }
        paramList.clear();
