...
%start CompUnit
%token CONST INT VOID IF WHILE BREAK CONTINUE RETURN PRINTF SCANF MAIN
...
%type <defalt>FuncDef FuncFParams FuncFParam 
...
%type <defalt>FName Entry_Void Entry_Int Entry_Void_Para Entry_Int_Para Entry_Main
%type <defalt>ParaArr Arr WhileBegin BeforeStmt NewLabel WhileEnd AfterStmt AfterElse
...
OtherConstDef :
    Ident ConstArrIdex '=' ConstInitVal {
		...
		if(symbol[level].find($1) != symbol[level].end()){
		...
		if(!level){
			...
			if (!$2){
				...
				symbol[level][$1] = Var(Constint, sta[$4].val, offset);
			}
			//else ?
		}
    }
...
ConstArrIdex : /*empty*/ {$$ = 0;}
    |'[' ConstExp ']' ConstArrIdex{
		if($4) 
            sta[$$ = $4].val = sta[$2].val * sta[$4].val;
		else 
            sta[$$ = ++tot].val = sta[$2].val;
...
OtherVarDef :Ident ConstArrIdex{
		if(symbol[level].find($1) != symbol[level].end()){
		...
		if(!level){
			if(!$2){
				...
				sprintf (tmp, "\t.type\t%s, @object\n", $1); Assemble.push_back(tmp);
				sprintf (tmp, "\t.size\t%s, %d\n", $1, sta[$2].val * 4); Assemble.push_back(tmp);
				sprintf (tmp, "%s:\n", $1); Assemble.push_back(tmp);
				sprintf (tmp, "\t.zero\t%d\n", sta[$2].val * 4); Assemble.push_back(tmp);
				Assemble.push_back("\t.text\n");
				symbol[0][$1] = Var(Arr, 0, 1, sta[$2].dim);
			}
		}
		else{
			if(!$2){
				offset -= 4;
				Assemble.push_back("\tsubq\t$4, %rsp\n");
				symbol[level][$1] = Var(Int, 0, offset);
			}
			else{
				offset -= 4 * sta[$2].val;
				sprintf (tmp, "\tsubq\t$%d, %rsp\n", sta[$2].val * 4); Assemble.push_back(tmp);
				symbol[level][$1] = Var(Arr, 0, offset, sta[$2].dim);
			}
		}
    }
    | Ident ConstArrIdex '=' InitVal {
		...
		if(!level){
			if (!$2){
				...
				sprintf (tmp, "\t.long\t%d\n", sta[$4].val); Assemble.push_back(tmp);
				Assemble.push_back("\t.text\n");
				symbol[0][$1] = Var(Int, sta[$4].val, 1);
			}
			else {
				if($4){
					yyerror("Initializer Error");
			...
			if (!$2){
				...
				Assemble.push_back("\tsubq\t$4, %rsp\n");
				sprintf (tmp, "\tmovl\t%%edi, %d(%rbp)\n", offset); Assemble.push_back(tmp);
				symbol[level][$1] = Var(Int, sta[$4].val, offset);
			}
			else {
				if($4){
					yyerror("Initializer Error");
...
FuncDef : VOID FName '(' ')' Entry_Void Block{
    ...
    | VOID FName '(' error ')' Block{
    }
    | INT FName '(' error ')' Block{
    }
...
FuncFParam : INT Ident ParaArr{
        Para.push_back($2);
    }
    | INT WrongIdent ParaArr{
    }
...
Stmt : LVal '=' Exp ';'{
    ...
    | IF '(' Cond ')' NewLabel BeforeStmt Stmt AfterStmt %prec LOWER_THAN_ELSE{
...
LVal : Ident Arr{
		...
		if(!$2){
			for(int i = level; i >= 0; --i)
				if(symbol[i].find($1) != symbol[i].end()){
					used = true;
					Var cur = symbol[i][$1];
					$$ = ++tot;
...
PrimaryExp : '(' Exp ')'{
    ...
    // delete[] buffer;       
        ...
        sta[$$ = ++tot].val = $1, sta[$$].is_const = true;          
    }
...
UnaryExp : PrimaryExp{
    ...
    | Ident '(' ')'{
		...
		if(symbol[0].find($1) == symbol[0].end()){
